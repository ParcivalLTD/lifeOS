  ...
  return (
    <main className="mx-auto flex w-full max-w-[1280px] flex-col gap-3 p-4">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(320px,1fr))] items-start gap-3">
        ...
        {data.courses.map((c) => (
          <section key={c.id} className="border border-border-outer bg-surface">
            ...
            {c.assessments.map((a) => {
              const dueUngraded = a.grade == null && a.dueISO <= data.todayISO;
              return (
                <div key={a.id} className="flex items-baseline gap-2.5 border-b border-border-row px-3 py-2">
                  ...
                  >
                    {a.dueISO === data.todayISO ? "DUE TODAY" : dueLabel(a.dueISO)}
                  </span>
            ...
            <form action={addAssessmentAction} className="flex flex-wrap gap-1.5 bg-subtle p-2.5">
              ...
              <input type="date" name="due" required defaultValue={data.todayISO} aria-label="Due date" className={`${inputCls} py-1.5 font-mono text-[11px]`} />
        ...
        <div className="flex flex-col gap-3">
          ...
          <Panel label="Study hours — this week" value="PLANNED VS ACTUAL">
            {data.study.length === 0 && (
              <p className="px-3 py-2 font-mono text-[10px] uppercase tracking-[.06em] text-faint">
            ...
            <form action={logStudyAction} className="flex flex-wrap gap-1.5 border-b border-border-row p-2.5">
            ...
            <div className="px-3 py-2 font-mono text-[10px] uppercase tracking-[.03em] text-faint">
              {data.paceLine}
            </div>
          </Panel>
          ...
          {/* add course */}
          <Panel label="Add course">
            <form action={addCourseAction} className="flex flex-wrap gap-1.5 p-3">
            ...
            <p className="px-3 pb-3 font-mono text-[9px] uppercase tracking-[.06em] text-faintest">
              Link a course to its goal from the course page — nesting lives in the goal engine.
            </p>
          </Panel>
        </div>
      </div>
